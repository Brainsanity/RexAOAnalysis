classdef SizePerceptBlock < RexBlock
	%% Size Perception task
	
	properties ( SetAccess = public, GetAccess = public )
		blockType = 'Size Perception Task';
		set_points = [];
		recorded_points = [];
	end

	methods
		%% SizePerceptBlock: function description
		function obj = SizePerceptBlock( blockName, data, dataSrc, fieldsFlag )
			if( nargin() < 1 )
				blockName = [];
			end
			if( nargin() <= 1 )
			...
			%% size perception task specified code
			for( i = 1 : obj.nTrials )
				switch( obj.trials(i).type )
					case { TRIAL_TYPE_DEF.CORRECT, TRIAL_TYPE_DEF.ERROR }
						if( size(obj.trials(i).fp,2) ~= 1 || obj.trials(i).fp.tOn < 0 || obj.trials(i).fp.nPats ~= 1 ||...
							...
							size(obj.trials(i).jmp1,2) ~= 1 || obj.trials(i).jmp1.tOn < 0 || obj.trials(i).fp.nPats ~= 1 ||...
