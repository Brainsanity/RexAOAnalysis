classdef CCueBlock < RexBlock
	%% color cue task
	
	properties ( SetAccess = public, GetAccess = public )
		blockType = 'Color Cue Task';
	end

	methods
		%% CCueBlock: function description
		function obj = CCueBlock( blockName, data, dataSrc, fieldsFlag, hasCue )
			if( nargin() < 1 )
				blockName = [];
			end
			if( nargin() <= 1 )
				...
				switch( obj.trials(i).type )
					...
					case TRIAL_TYPE_DEF.FIXBREAK
						...
						if( size(obj.trials(i).fp,2) ~= 1 ||...
							size(obj.trials(i).rf,2) ~= 1 || all( obj.trials(i).rf.red > 0 ) ||...
